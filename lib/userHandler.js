...
obj.userChatInput = (data, callback) => {
    ...
    conn.query("insert into " + fn.getTableName('chat') + "(sender_id, channel_id, message, date_created) values(?,?,AES_ENCRYPT(?, ?),?)", [data.senderId, data.channelId, data.message, encryptionKey, date]).then((data1) => {
        conn.query("select t2.name as userName,t1.id,AES_DECRYPT(t1.message, '" + encryptionKey + "') as message,t1.date_created,t1.channel_id as channelId,t1.sender_id as senderId,t1.is_seen,'https://121directory.com/data/profile.jpg' as image from " + fn.getTableName('chat') + " as t1 left join " + userTableName + " as t2 on t2.id=t1.sender_id where t1.channel_id='" + data.channelId + "' and t1.id='" + data1.insertId + "' LIMIT 1").then((data1) => {
